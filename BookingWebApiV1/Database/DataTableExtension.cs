...
public static class DataTableExtension
{
    ...
    public static int ConvertDataTableToEffectKwh(DataTable dataTable)
    {
        int effect = -1;

        foreach (DataRow row in dataTable.Rows)
        {
            effect = int.Parse((string)row["EffectKWh"]);
        }

        return effect;
    }

    public static List<BookingElectricityPriceDTO> ConvertDataTableToBookingElectricityPriceDTO(DataTable dataTable)
    {
    ...
    // p.ProgramId, p.ProgramName, p.ProgramRunTimeMinutes, m.MachineManufacturer, m.ModelName, m.EffectKWh
